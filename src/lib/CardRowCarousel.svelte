<script lang="ts">
  ...
  // This calculation allows the page to handle the gap on subsequent pages
  // and correctly align the cards to the beginning of the frame
  $: transformPercent = `calc(${pageIndex * -100}% - (${
    spacing * 1.5
  }px * ${pageIndex}) )`;
